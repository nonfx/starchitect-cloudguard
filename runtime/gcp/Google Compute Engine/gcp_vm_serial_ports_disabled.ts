...
function isSerialPortEnabled(instance: any): boolean {
	return (
		instance.metadata?.items?.some(
			(item: any) => item.key === "serial-port-enable" && item.value?.toUpperCase() === "TRUE"
		) ?? false
	);
}
...
export default {
	...
	description:
		"VM instances should not have serial port access enabled as it allows connections from any IP address without IP-based restrictions.",
	controls: [
		{
