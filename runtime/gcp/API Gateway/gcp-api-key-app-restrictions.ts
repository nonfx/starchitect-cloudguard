...
function hasAppRestrictions(key: any): boolean {
  ...
  const hasValidHttpReferrers = key.restrictions.browserKeyRestrictions?.allowedReferrers?.length > 0 &&
  ...
  const hasValidIpRestrictions = key.restrictions.serverKeyRestrictions?.allowedIps?.length > 0 &&
  ...
  const hasAndroidRestrictions = key.restrictions.androidKeyRestrictions?.allowedApplications?.length > 0;
  const hasIosRestrictions = key.restrictions.iosKeyRestrictions?.allowedBundleIds?.length > 0;

  return hasValidHttpReferrers || hasValidIpRestrictions || hasAndroidRestrictions || hasIosRestrictions;
}
