...
async function checkNeptuneCopyTagsToSnapshot(
	...
	try {
		...
		// Check each cluster for tag copying configuration
		for (const cluster of response.DBClusters) {
			...
			const copyTagsEnabled = cluster.CopyTagsToSnapshot ?? false;
...
if (import.meta.main) {
	...
	const results = await checkNeptuneCopyTagsToSnapshot(region);
...
export default {
	...
	description:
		"Neptune DB clusters must be configured to automatically copy all tags to snapshots for consistent metadata and access policies.",
	controls: [
		{
			id: "AWS-Foundational-Security-Best-Practices_v1.0.0_Neptune.8",
			document: "AWS-Foundational-Security-Best-Practices_v1.0.0"
		}
	],
	severity: "HIGH",
	execute: checkNeptuneCopyTagsToSnapshot,
