...
describe('checkSecurityHubEnabled', () => {
    ...
    describe('Compliant Resources', () => {
        it('should return PASS when Security Hub is enabled and active', async () => {
            ...
            const result = await checkSecurityHubEnabled('us-east-1');
    ...
    describe('Metadata', () => {
        it('should include correct metadata in results', async () => {
            ...
            expect(result.metadoc.title).toBe('Ensure AWS Security Hub is enabled');
            expect(result.metadoc.controls).toHaveLength(1);
            expect(result.metadoc.controls[0].id).toBe('CIS-AWS-Foundations-Benchmark_v3.0.0_4.16');
        });
    });
});