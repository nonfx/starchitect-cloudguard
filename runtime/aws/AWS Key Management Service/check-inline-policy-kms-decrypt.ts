import { IAMClient, ListUsersCommand, ListRolesCommand, ListGroupsCommand, ListUserPoliciesCommand, ListRolePoliciesCommand, ListGroupPoliciesCommand, GetUserPolicyCommand, GetRolePolicyCommand, GetGroupPolicyCommand } from '@aws-sdk/client-iam';
...
async function checkInlinePolicyKmsDecrypt(region: string = 'us-east-1'): Promise<ComplianceReport> {
  ...
  try {
    ...
    for (const user of users.Users || []) {
      ...
      const userPolicies = await client.send(new ListUserPoliciesCommand({
        ...
        try {
          ...
          if (policy.PolicyDocument) {
            ...
            results.checks.push({
              ...
              message: hasViolation ? 'Inline policy allows KMS decrypt actions on all keys' : undefined
            });
          }
        } catch (error) {
          results.checks.push({
    ...
    for (const role of roles.Roles || []) {
      ...
      const rolePolicies = await client.send(new ListRolePoliciesCommand({
    ...
    for (const group of groups.Groups || []) {
      ...
      const groupPolicies = await client.send(new ListGroupPoliciesCommand({
...
export default checkInlinePolicyKmsDecrypt;