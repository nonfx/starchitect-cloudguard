...
describe("checkKmsKeyRotation", () => {
    ...
    describe("Compliant Resources", () => {
        it("should return PASS when key rotation is enabled for customer-managed symmetric keys", async () => {
            ...
            mockKmsClient
                ...
                .resolves({ KeyMetadata: mockCustomerManagedKey });
            mockKmsClient
                .on(GetKeyRotationStatusCommand)
                .resolves({ KeyRotationEnabled: true });

            const result = await checkKmsKeyRotation("us-east-1");
        ...
        it("should skip AWS managed keys", async () => {
            mockKmsClient
                .on(ListKeysCommand)
                .resolves({ Keys: [{ KeyId: mockAwsManagedKey.KeyId }] });
            mockKmsClient
                .on(DescribeKeyCommand)
                .resolves({ KeyMetadata: mockAwsManagedKey });
        ...
        it("should skip asymmetric keys", async () => {
            mockKmsClient
                .on(ListKeysCommand)
                .resolves({ Keys: [{ KeyId: mockAsymmetricKey.KeyId }] });
            mockKmsClient
                .on(DescribeKeyCommand)
                .resolves({ KeyMetadata: mockAsymmetricKey });
    ...
    describe("Error Handling", () => {
        ...
        it("should return ERROR when ListKeys fails", async () => {
            mockKmsClient
                .on(ListKeysCommand)
                .rejects(new Error("Failed to list keys"));
