...
describe("checkEcsContainerInsights", () => {
    ...
    describe("Compliant Resources", () => {
        it("should return PASS when Container Insights is enabled", async () => {
            mockEcsClient
                ...
                .resolves({ clusterArns: [mockClusterArns[0]] });
            mockEcsClient
                .on(DescribeClustersCommand)
                .resolves({ clusters: [mockClusterWithInsights] });

            const result = await checkEcsContainerInsights("us-east-1");
    ...
    describe("Non-Compliant Resources", () => {
        it("should return FAIL when Container Insights is disabled", async () => {
            mockEcsClient
                .on(ListClustersCommand)
                .resolves({ clusterArns: [mockClusterArns[1]] });
            mockEcsClient
                .on(DescribeClustersCommand)
        ...
        it("should handle mixed compliance results", async () => {
            mockEcsClient
                .on(ListClustersCommand)
                .resolves({ clusterArns: mockClusterArns });
            mockEcsClient
                .on(DescribeClustersCommand)
    ...
    describe("Error Handling", () => {
        it("should return ERROR when ListClusters fails", async () => {
            mockEcsClient
                .on(ListClustersCommand)
                .rejects(new Error("Failed to list clusters"));
