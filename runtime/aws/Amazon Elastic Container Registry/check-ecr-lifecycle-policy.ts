import { ECRClient, DescribeRepositoriesCommand, GetLifecyclePolicyCommand } from "@aws-sdk/client-ecr";
...
async function checkEcrLifecyclePolicyCompliance(region: string = "us-east-1"): Promise<ComplianceReport> {
    ...
    for (const repo of repositories.repositories) {
      ...
      try {
        ...
        results.checks.push({
          ...
          message: hasPolicy ? undefined : "ECR repository does not have a lifecycle policy configured"
        });
      } catch (error) {
        results.checks.push({
          resourceName: repository.name,
...
export default checkEcrLifecyclePolicyCompliance;