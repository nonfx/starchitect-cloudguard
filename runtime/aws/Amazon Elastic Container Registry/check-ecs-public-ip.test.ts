...
describe("checkEcsPublicIp", () => {
    ...
    describe("Compliant Resources", () => {
        it("should return PASS when service has public IP disabled", async () => {
            ...
            mockEcsClient
                ...
                .resolves({ services: [mockServices[0]] });
    ...
    describe("Non-Compliant Resources", () => {
        it("should return FAIL when service has public IP enabled", async () => {
            ...
            mockEcsClient
                .on(DescribeServicesCommand)
                .resolves({ services: [mockServices[1]] });

            const result = await checkEcsPublicIp.execute("us-east-1");
            expect(result.checks[0].status).toBe(ComplianceStatus.FAIL);
            expect(result.checks[0].message).toBe("ECS service has automatic public IP assignment enabled");
        });
        ...
        it("should handle mixed compliance results", async () => {
            ...
            mockEcsClient
                .on(DescribeServicesCommand)
                .resolves({ services: mockServices });
    ...
    describe("Error Handling", () => {
        ...
        it("should handle pagination", async () => {
            const serviceArns = Array(15).fill(null).map((_, i) => `service-${i}`);
            mockEcsClient
                .on(ListServicesCommand)
                .resolves({ serviceArns });
            mockEcsClient
