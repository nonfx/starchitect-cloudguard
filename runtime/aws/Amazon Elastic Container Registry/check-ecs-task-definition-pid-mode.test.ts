//@ts-nocheck
import { ECSClient, DescribeTaskDefinitionCommand, ListTaskDefinitionsCommand } from '@aws-sdk/client-ecs';
...
describe('checkEcsTaskDefinitionPidMode', () => {
    ...
    describe('Compliant Resources', () => {
        it('should return PASS when task definition has no pidMode set', async () => {
            ...
            const result = await checkEcsTaskDefinitionPidMode('us-east-1');
        ...
        it('should return PASS when task definition has pidMode not set to host', async () => {
            ...
            mockECSClient
                .on(DescribeTaskDefinitionCommand)
                .resolves({ taskDefinition: { taskDefinitionArn: mockTaskDefinitionArn1, pidMode: 'task' } });
    ...
    describe('Non-Compliant Resources', () => {
        it('should return FAIL when task definition has pidMode set to host', async () => {
            ...
            mockECSClient
                .on(DescribeTaskDefinitionCommand)
                .resolves({ taskDefinition: { taskDefinitionArn: mockTaskDefinitionArn1, pidMode: 'host' } });

            const result = await checkEcsTaskDefinitionPidMode('us-east-1');
            expect(result.checks[0].status).toBe(ComplianceStatus.FAIL);
            expect(result.checks[0].message).toBe('Task definition shares host process namespace (pidMode: host)');
        });
