...
describe("checkInitialAccessKeys", () => {
	...
	describe("Non-Compliant Resources", () => {
		...
		it("should handle multiple users with mixed compliance", async () => {
			...
			mockIAMClient
				.on(GetLoginProfileCommand, { UserName: "user-1" })
				.resolves({})
				.on(GetLoginProfileCommand, { UserName: "user-2" })
				.resolves({});
			mockIAMClient
				.on(ListAccessKeysCommand, { UserName: "user-1" })
				.resolves({ AccessKeyMetadata: [{ AccessKeyId: "AKIA123" }] })
				.on(ListAccessKeysCommand, { UserName: "user-2" })
				.resolves({ AccessKeyMetadata: [] });
