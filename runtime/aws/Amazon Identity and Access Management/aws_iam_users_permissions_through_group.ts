...
async function checkIamUserPermissionsThroughGroups(region: string = 'us-east-1'): Promise<ComplianceReport> {
  ...
  try {
    ...
    for (const user of listUsersResponse.Users) {
      ...
      try {
        ...
        const inlinePoliciesResponse = await client.send(new ListUserPoliciesCommand({
          UserName: user.UserName
        }));

        // Check for attached policies
        const attachedPoliciesResponse = await client.send(new ListAttachedUserPoliciesCommand({
          UserName: user.UserName
        }));

        // Check for group memberships
        const groupsResponse = await client.send(new ListGroupsForUserCommand({
          UserName: user.UserName
        }));
