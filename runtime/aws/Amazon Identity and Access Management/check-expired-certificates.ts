import { IAMClient, ListServerCertificatesCommand, type ServerCertificateMetadata } from '@aws-sdk/client-iam';
...
async function checkExpiredCertificates(region: string = 'us-east-1'): Promise<ComplianceReport> {
  ...
  try {
    ...
    if (!response.ServerCertificateMetadataList || response.ServerCertificateMetadataList.length === 0) {
      results.checks = [{
