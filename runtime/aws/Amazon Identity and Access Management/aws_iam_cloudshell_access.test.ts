import { IAMClient, ListUsersCommand, ListGroupsCommand, ListRolesCommand, ListAttachedUserPoliciesCommand, ListAttachedGroupPoliciesCommand, ListAttachedRolePoliciesCommand } from "@aws-sdk/client-iam";
...
describe("checkCloudShellAccess", () => {
    ...
    describe("Compliant Resources", () => {
        it("should return PASS when no entities have CloudShell access", async () => {
            ...
            const result = await checkCloudShellAccess();
            expect(result.checks).toHaveLength(1);
            expect(result.checks[0].status).toBe(ComplianceStatus.PASS);
            expect(result.checks[0].message).toBe("No entities with AWSCloudShellFullAccess policy found");
        });
    ...
    describe("Non-Compliant Resources", () => {
        it("should return FAIL when user has CloudShell access", async () => {
            ...
            expect(result.checks[0].message).toContain("User has AWSCloudShellFullAccess policy attached");
        });

        it("should return FAIL when group has CloudShell access", async () => {
            mockIAMClient
                .on(ListUsersCommand).resolves({ Users: [] })
                .on(ListGroupsCommand).resolves({ 
                    Groups: [{ GroupName: "test-group", Arn: "arn:aws:iam::123456789012:group/test-group" }] 
                })
                .on(ListAttachedGroupPoliciesCommand).resolves({ 
                    AttachedPolicies: [{ PolicyArn: CLOUDSHELL_FULL_ACCESS_ARN }] 
                })
                .on(ListRolesCommand).resolves({ Roles: [] });
            ...
            expect(result.checks).toHaveLength(1);
            expect(result.checks[0].status).toBe(ComplianceStatus.FAIL);
            expect(result.checks[0].message).toContain("Group has AWSCloudShellFullAccess policy attached");
        });
        ...
        it("should return FAIL when role has CloudShell access", async () => {
            mockIAMClient
                .on(ListUsersCommand).resolves({ Users: [] })
                .on(ListGroupsCommand).resolves({ Groups: [] })
                .on(ListRolesCommand).resolves({ 
                    Roles: [{ RoleName: "test-role", Arn: "arn:aws:iam::123456789012:role/test-role" }] 
                })
                .on(ListAttachedRolePoliciesCommand).resolves({ 
                    AttachedPolicies: [{ PolicyArn: CLOUDSHELL_FULL_ACCESS_ARN }] 
                });
            ...
            expect(result.checks).toHaveLength(1);
            expect(result.checks[0].status).toBe(ComplianceStatus.FAIL);
            expect(result.checks[0].message).toContain("Role has AWSCloudShellFullAccess policy attached");
        });
