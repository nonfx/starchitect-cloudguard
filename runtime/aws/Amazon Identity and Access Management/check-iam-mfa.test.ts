//@ts-nocheck
import { IAMClient, ListUsersCommand, GetLoginProfileCommand, ListMFADevicesCommand } from "@aws-sdk/client-iam";
...
describe("checkIamMfa", () => {
    ...
    describe("Compliant Resources", () => {
        it("should return PASS for user with console access and MFA enabled", async () => {
            ...
            mockIAMClient.on(ListMFADevicesCommand).resolves({ MFADevices: [{ SerialNumber: "mfa-device" }] });
    ...
    describe("Non-Compliant Resources", () => {
        ...
        it("should handle multiple users with mixed compliance", async () => {
            ...
            mockIAMClient
                .on(GetLoginProfileCommand, { UserName: "user-with-mfa" }).resolves({})
                .on(GetLoginProfileCommand, { UserName: "user-without-mfa" }).resolves({})
                .on(GetLoginProfileCommand, { UserName: "user-no-console" }).rejects({ name: "NoSuchEntity" });
            mockIAMClient
                .on(ListMFADevicesCommand, { UserName: "user-with-mfa" }).resolves({ MFADevices: [{ SerialNumber: "mfa-device" }] })
                .on(ListMFADevicesCommand, { UserName: "user-without-mfa" }).resolves({ MFADevices: [] })
                .on(ListMFADevicesCommand, { UserName: "user-no-console" }).resolves({ MFADevices: [] });
