...
async function checkBeanstalkPersistentLogs(region: string = 'us-east-1'): Promise<ComplianceReport> {
  ...
  try {
    ...
    for (const env of environments.Environments) {
      ...
      try {
        ...
        const streamLogs = settings.find(s => 
          s.Namespace === 'aws:elasticbeanstalk:cloudwatch:logs' && 
          s.OptionName === 'StreamLogs'
        );
        const retentionDays = settings.find(s => 
          s.Namespace === 'aws:elasticbeanstalk:cloudwatch:logs' && 
          s.OptionName === 'RetentionInDays'
        );
        const deleteOnTerminate = settings.find(s => 
          s.Namespace === 'aws:elasticbeanstalk:cloudwatch:logs' && 
        ...
        if (!isCompliant) {
          ...
          if (!retentionDays?.Value || parseInt(retentionDays.Value) <= 0) issues.push('Log retention is not configured');
          if (deleteOnTerminate?.Value !== 'false') issues.push('Logs are not kept after termination');
          message = issues.join(', ');
        }
