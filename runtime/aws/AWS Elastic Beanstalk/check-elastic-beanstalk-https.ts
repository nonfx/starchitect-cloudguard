...
async function checkElasticBeanstalkHttpsCompliance(region: string = 'us-east-1'): Promise<ComplianceReport> {
      ...
      try {
        ...
        if (configSettings.ConfigurationSettings) {
          for (const config of configSettings.ConfigurationSettings) {
            const httpsSettings = config.OptionSettings?.find(setting =>
