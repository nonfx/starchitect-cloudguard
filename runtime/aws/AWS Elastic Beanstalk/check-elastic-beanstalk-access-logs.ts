...
async function checkElasticBeanstalkAccessLogs(region: string = 'us-east-1'): Promise<ComplianceReport> {
  ...
  try {
    ...
    for (const env of environments.Environments) {
      ...
      try {
        ...
        const accessLogsEnabled = settings.some(s =>
        ...
        results.checks.push({
          ...
          message: accessLogsEnabled ? undefined : 'Access logs are not enabled for the load balancer'
        });
      } catch (error) {
        results.checks.push({
          resourceName: env.EnvironmentName,
