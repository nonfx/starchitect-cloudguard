...
async function checkCloudFrontOACCompliance(region: string = "us-east-1"): Promise<ComplianceReport> {
  ...
  try {
    ...
    for (const distribution of response.DistributionList.Items) {
      ...
      try {
        ...
        const s3Origins = origins.filter(origin => 
          origin.DomainName?.includes(".s3.") || 
