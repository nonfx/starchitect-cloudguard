...
const mockLaunchConfigWithPublicIP = {
	...
	LaunchConfigurationARN:
		"arn:aws:autoscaling:us-east-1:123456789012:launchConfiguration:test-config-1",
...
const mockLaunchConfigWithoutPublicIP = {
	...
	LaunchConfigurationARN:
		"arn:aws:autoscaling:us-east-1:123456789012:launchConfiguration:test-config-2",
...
describe("checkAutoScalingPublicIp", () => {
	...
	describe("Compliant Resources", () => {
		it("should return PASS when launch configuration does not assign public IPs", async () => {
			...
			expect(result.checks[0].resourceArn).toBe(
				mockLaunchConfigWithoutPublicIP.LaunchConfigurationARN
			);
		});
	...
	describe("Pagination and Error Handling", () => {
		...
		it("should return ERROR when API call fails", async () => {
			mockAutoScalingClient.on(DescribeLaunchConfigurationsCommand).rejects(new Error("API Error"));
