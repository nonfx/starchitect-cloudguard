  ...
  try {
    ...
    for (const template of templatesResponse.LaunchTemplates) {
      ...
      try {
        ...
        const versionResponse = await client.send(new DescribeLaunchTemplateVersionsCommand({
        ...
        results.checks.push({
          ...
          message: isCompliant ? undefined : 'Launch template does not require IMDSv2 (HttpTokens is not set to required)'
        });
      } catch (error) {
        results.checks.push({
          resourceName: template.LaunchTemplateName,
