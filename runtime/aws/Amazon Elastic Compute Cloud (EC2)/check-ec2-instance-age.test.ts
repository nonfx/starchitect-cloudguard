	...
	describe("Compliant Resources", () => {
		it("should return PASS for instances less than 180 days old", async () => {
			...
			mockEC2Client.on(DescribeInstancesCommand).resolves({
				Reservations: [
					{
						Instances: [createMockInstance("i-123456789", recentDate)]
					}
				]
			});

			const result = await checkEc2InstanceAge.execute("us-east-1");
	...
	describe("Non-Compliant Resources", () => {
		it("should return FAIL for instances older than 180 days", async () => {
			...
			mockEC2Client.on(DescribeInstancesCommand).resolves({
				Reservations: [
					{
						Instances: [createMockInstance("i-987654321", oldDate)]
					}
				]
			});
