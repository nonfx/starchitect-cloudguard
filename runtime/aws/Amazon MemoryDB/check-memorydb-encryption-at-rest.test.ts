...
import checkMemoryDBEncryption from "./check-memorydb-encryption-at-rest";
...
describe("checkMemoryDBEncryption", () => {
	...
	describe("Compliant Resources", () => {
		it("should return PASS when cluster is encrypted at rest", async () => {
	...
	describe("Non-Compliant Resources", () => {
		it("should return FAIL when cluster is not encrypted at rest", async () => {
			mockMemoryDBClient.on(DescribeClustersCommand).resolves({
			...
			expect(result.checks[0].message).toBe("MemoryDB cluster is not encrypted at rest");
		});
