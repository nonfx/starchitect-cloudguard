	...
	try {
		...
		if (!matchingFilter) {
			results.checks.push({
				...
				status: ComplianceStatus.FAIL,
				message: "CloudTrail log group does not have required security group changes metric filter"
			});
			return results;
		}
		...
		if (!alarms.MetricAlarms || alarms.MetricAlarms.length === 0) {
			results.checks.push({
				...
				resourceArn: logGroupArn,
				status: ComplianceStatus.FAIL,
				message: "No alarm configured for security group changes metric filter"
			});
		} else {
			results.checks.push({
				resourceName: logGroupName,
