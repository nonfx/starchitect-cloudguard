...
const SECURITY_GROUP_PATTERN = '{ ($.eventName = AuthorizeSecurityGroupIngress) || ' +
...
describe('checkSecurityGroupMonitoring', () => {
    ...
    it('should return FAIL when no metric filter exists', async () => {
        ...
        const result = await checkSecurityGroupMonitoring.execute('us-east-1');
        expect(result.checks[0].status).toBe(ComplianceStatus.FAIL);
        expect(result.checks[0].message).toContain('Log group does not have required security group changes metric filter');
    });
