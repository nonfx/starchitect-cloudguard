...
describe("checkConfigEnabledAllRegions", () => {
	...
	describe("Compliant Resources", () => {
		it("should return PASS when account aggregation is enabled for all regions", async () => {
			...
			expect(result.checks[0].message).toBe(
				"Config aggregator is properly configured for all regions"
			);
		});
	...
	describe("Non-Compliant Resources", () => {
		it("should return FAIL when no aggregators exist", async () => {
			...
			expect(result.checks[0].message).toBe(
				"No configuration aggregators found. AWS Config might not be enabled."
			);
		});
	...
	describe("Error Handling", () => {
		it("should return ERROR when API call fails", async () => {
			mockConfigClient.on(DescribeConfigurationAggregatorsCommand).rejects(new Error("API Error"));
