package rules.aws_cloudwatch_config
...
__rego__metadoc__ := {
	...
	"custom": {"controls": {"CIS-AWS-Foundations-Benchmark_v3.0.0": ["CIS-AWS-Foundations-Benchmark_v3.0.0_4.9"]}, "severity": "High", "reviewer": "ssghait.007@gmail.com"},
}
...
required_pattern := "{ ($.eventSource = config.amazonaws.com) && (($.eventName=StopConfigurationRecorder)||($.eventName=DeleteDeliveryChannel) ||($.eventName=PutDeliveryChannel)||($.eventName=PutConfigurationRecorder)) }"
...
policy[p] {
	...
	msg := sprintf("CloudWatch Log Group '%s' does not have the required metric filter for monitoring AWS Config configuration changes", [log_group.name])
...
policy[p] {
	...
	msg := sprintf("CloudWatch Metric Filter '%s' does not have an associated alarm for monitoring AWS Config configuration changes", [metric_filter.name])
