package rules.aws_cloudwatch_api_monitoring
...
__rego__metadoc__ := {
	...
	"custom": {"controls": {"CIS-AWS-Foundations-Benchmark_v3.0.0": ["CIS-AWS-Foundations-Benchmark_v3.0.0_4.1"]}, "severity": "Medium", "reviewer": "ssghait.007@gmail.com"},
}
...
policy[p] {
	...
	msg := sprintf("CloudWatch Log Group '%s' does not have the required metric filter for monitoring management unauthorized API calls changes", [log_group.name])
...
policy[p] {
	...
	msg := sprintf("CloudWatch Metric Filter '%s' does not have an associated alarm for monitoring management unauthorized API calls changes", [metric_filter.name])
