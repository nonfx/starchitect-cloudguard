package rules.aws_cloudwatch_cloudtrail
...
__rego__metadoc__ := {
	...
	"custom": {"controls": {"CIS-AWS-Foundations-Benchmark_v3.0.0": ["CIS-AWS-Foundations-Benchmark_v3.0.0_4.5"], "AWS-Foundational-Security-Best-Practices_v1.0.0": ["AWS-Foundational-Security-Best-Practices_v1.0.0_CloudTrail.5"]}, "severity": "Medium", "reviewer": "ssghait.007@gmail.com"},
}
...
required_pattern := "{ ($.eventName = CreateTrail) || ($.eventName = UpdateTrail) || ($.eventName = DeleteTrail) || ($.eventName = StartLogging) || ($.eventName = StopLogging) }"
...
policy[p] {
	...
	msg := sprintf("CloudWatch Log Group '%s' does not have the required metric filter for monitoring CloudTrail configuration changes", [log_group.name])
...
policy[p] {
	...
	msg := sprintf("CloudWatch Metric Filter '%s' does not have an associated alarm for monitoring CloudTrail configuration changes", [metric_filter.name])
