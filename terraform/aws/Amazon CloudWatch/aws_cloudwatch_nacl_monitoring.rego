package rules.aws_cloudwatch_nacl_monitoring
...
__rego__metadoc__ := {
	...
	"custom": {"controls": {"CIS-AWS-Foundations-Benchmark_v3.0.0": ["CIS-AWS-Foundations-Benchmark_v3.0.0_4.11"]}, "severity": "Low", "reviewer": "ssghait.007@gmail.com"},
}
...
required_pattern := "{ ($.eventName = CreateNetworkAcl) || ($.eventName = CreateNetworkAclEntry) || ($.eventName = DeleteNetworkAcl) || ($.eventName = DeleteNetworkAclEntry) || ($.eventName = ReplaceNetworkAclEntry) || ($.eventName = ReplaceNetworkAclAssociation) }"
...
policy[p] {
	...
	msg := sprintf("CloudWatch Log Group '%s' does not have the required metric filter for monitoring Network Access Control Lists (NACL) changes", [log_group.name])
...
policy[p] {
	...
	msg := sprintf("CloudWatch Metric Filter '%s' does not have an associated alarm for monitoring Network Access Control Lists (NACL) changes", [metric_filter.name])
