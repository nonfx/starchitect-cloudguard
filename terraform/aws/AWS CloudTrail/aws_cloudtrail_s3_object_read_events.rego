package rules.aws_cloudtrail_s3_object_read_events
...
valid_read_write_types := ["ReadOnly", "All"]
...
policy[p] {
	...
	p = fugue.deny_resource_with_message(bucket, sprintf("S3 bucket '%s' does not have object-level logging enabled for read events in any CloudTrail", [bucket.id]))
}
