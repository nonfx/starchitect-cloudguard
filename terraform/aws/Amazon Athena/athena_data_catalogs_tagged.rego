package rules.athena_data_catalogs_tagged
...
# Policy rule
policy[p] {
	catalog := athena_data_catalogs[_]
	has_tags(catalog)
	p = fugue.allow_resource(catalog)
}

policy[p] {
	catalog := athena_data_catalogs[_]
	not has_tags(catalog)
	p = fugue.deny_resource_with_message(catalog, "Athena data catalog does not have any non-system tags")
}
